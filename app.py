...
# --- FÓRMULA DE PONTUAÇÃO (PADRÃO INTERNACIONAL) ---
def calcular_pontos_dinamico(index_linear):
    pos = index_linear + 1
    if pos == 1: return 100
    if pos == 2: return 95
    # Decréscimo de 1 ponto por posição
    pontuacao = 95 - (pos - 2)
...
with aba1:
        ...
        if st.button("GERAR PRÉVIA"):
            if txt_input:
                ...
                for l in txt_input.strip().split('\n'):
                    try:
                        partes = l.replace("'", " ").replace(":", " ").replace("+", " ").strip().split()
                        if len(partes) >= 1:
                            nome_limpo = " ".join([p for p in partes if not p.isdigit()]).upper().replace("-", "").strip()
                            ...
                            if nome_limpo: dados.append({"Data": data_treino.strftime("%d/%m/%Y"), "Horario": horario_sel, "Nome": nome_limpo, "Tempo": tempo_str, "Segundos": seg_total})
                    except: continue
    ...
    with col_b:
        if st.button("🗑️ LIMPAR"):
            st.session_state.input_texto = ""; st.session_state.show_preview = False; st.rerun()
    ...
    if st.session_state.get("show_preview"):
        ...
        if st.button("🚀 SALVAR RESULTADOS"):
            if requests.post(URL_GOOGLE_SCRIPT, json=st.session_state.ready_to_save).status_code == 200:
                st.success("Salvo!"); st.session_state.input_texto = ""; st.session_state.show_preview = False; st.cache_data.clear(); st.rerun()
...
with aba2:
    ...
    if not df_hist.empty:
        ...
        h_filtro = st.selectbox("Filtro Horário:", ["Todos"] + h_disp)
...
with aba3:
    ...
    if not df_geral.empty:
        # Garante que nomes iguais sejam somados juntos
        df_geral['Nome'] = df_geral['Nome'].astype(str).str.upper().str.strip()
        
        lista_acum = []
        for d in df_geral["Data"].unique():
            dia = df_geral[df_geral["Data"] == d].copy().sort_values("Segundos").reset_index(drop=True)
            # CHAMADA CORRETA DA FUNÇÃO AQUI:
        ...
        # Ordena por Pontos (PTS) decrescente e WODs (WDS) decrescente
